...
**TYPESCRIPT SUPPORT**
Unfortunately styled-system does not have full typescript support for all of this features, so this module, at the moment, is based on a custom `styled-system.d.ts` file. The optimal approach would be to totally fork styled-system and add the a built-in definition file

**HOW IT WORKS**

In order to work, styled-system requires:
 1. A theme object respecting a specific schema
 2. The values of that object being of type `ObjectOrArray`, which is an array with custom properties [https://styled-system.com/responsive-styles/#using-objects](https://styled-system.com/responsive-styles/#using-objects)

So the main purpose of our module is to 
...
- Provide a custom `ThemeProvider` that:
	...
	3) returns a styled-components `ThemeProvider` instance (specific for web or mobile environment) which takes in input the previously generated theme 
